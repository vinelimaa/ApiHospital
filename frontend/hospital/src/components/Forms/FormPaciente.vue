<template>
	<div>
		<form class="form" id="form_paciente">
			...
			<input class="form-control" v-model="cpf" placeholder="CPF" @keypress="mask_cpf" type="text"
				required="required" autocomplete="off" maxlength="14" style="width: 40vh;" />
			<input class="form-control" @focus="verificarCPF" id="name" v-model="nome" placeholder="Nome Completo"
				type="text" required />

			<div style="display:flex">
				<input class="form-control" v-model="data_nascimento" placeholder="Data de Nascimento" type="date"
					required="required" maxlength="8" style="margin-right:5px;" />
			...
			<input class="form-control" v-model="cep" @blur="consultarCEP" placeholder="CEP" type="number"
				required="required" />
			<input class="form-control" v-model="logradouro" placeholder="Logradouro" type="text" required="required" />

			<div style="display:flex">
				<input class="form-control" v-model="numero" placeholder="Número" type="number" required="required"
					style="margin-right:5px;" />
				<input class="form-control" v-model="complemento" placeholder="Complemento" type="text"
					equired="required" style="margin-left:5px;" />
			</div>
			<input class="form-control" v-model="telefone" placeholder="Telefone" type="text" @keypress="mask_phone"
				autocomplete="off" maxlength="13" equired="required" />
			<button class="btn btn-primary" type="submit" id="submitPaciente" v-if="!editar"
				@click="cadastrarPaciente">Cadastrar</button>
			<button class="btn btn-primary" type="submit" id="submitPaciente" v-if="editar"
				@click="editarPaciente">Salvar</button>
			<button class="btn btn-primary" type="submit" id="Limpar" v-if="editar"
				style="margin-top:5px;">Voltar</button>

			<button class="btn btn-primary mt-2" @click.prevent="listar">Listar Pacientes</button>

		</form>
		<table class="bg-light lista_pacientes text-center m-auto" v-if="dados">
			<thead>
				<tr>
					<th width="50px" style="font-size: 13px;" class="text-center border border-dark p-1"
						v-for="value, index in dados[0] " :key="value">{{ index }}</th>
			...
			<tbody>
				<tr v-for="pacientes, index in dados" :key="index">
					<td width="50px" style="font-size: 13px;" class="text-center border border-dark p-1"
						v-for="paciente, index in pacientes" :key="index"> {{ paciente }}</td>
					<td width="50px" style="font-size: 13px;" class="text-center border border-dark p-1"><button
							class="btn btn-primary" v-if="pacientes" :value="pacientes.cpf"
							@click="getDados">Editar</button></td>
					<td width="50px" style="font-size: 13px;" class="text-center border border-dark p-1"><button
							class="btn btn-danger" @click="deletar" :id="pacientes.pacienteId">Deletar</button></td>
				</tr>
			</tbody>
		</table>
	</div>
...
function getDados(data) {
	...
	verificarCPF(data.target.value)
}
