	...
	<div>
		<form class="form">
			...
			<div style="display: flex;">
				<input class="form-control" v-model="cpf" placeholder="CPF" type="text" required="required"
					@keypress="mask_cpf" @blur="buscarPacienteCPF" style="width: 30vh;" />
				<input class="form-control" v-model="nome" type="text" style="margin-left:5px;" v-if="get"
					required="required" />
			</div>

			<div style="display:flex">
				<input class="form-control" v-model="dataInt" placeholder="Data de Internamento" type="date"
					required="required" style="margin-right:5px;" />
				<input class="form-control" v-model="dataAlta" placeholder="Data de Alta" type="date"
					required="required" style="margin-left:5px;" />
			</div>
			...
			<button class="btn btn-primary" type="submit" id="submitPaciente" v-if="!editar"
				@click.prevent='cadastrarInternamento'>Cadastrar Internamento</button>
			<button class="btn btn-primary" type="submit" id="editarPaciente" v-if="editar"
				@click.prevent="editarInternamento">Editar</button>
		...
		<table class="bg-light table_pacientes text-center m-auto " v-if="dados" @load="listar">
			...
			<tbody>
				<tr v-for="internamentos, index in dados" :key="index">
					<td width="50px" style="font-size: 13px;" class="text-center border border-dark p-1" v-for="internamento, index in internamentos" :key="index"> {{ internamento }}</td>
					<td width="50px" style="font-size: 13px;" class="text-center border border-dark p-1"><button class="btn btn-primary" :id="internamentos.internamentoId" @click="getInternanmentoId">Editar</button></td>
					<td width="50px" style="font-size: 13px;" class="text-center border border-dark p-1"><button class="btn btn-danger" @click="deletar" :id="internamentos.internamentoId">Deletar</button></td>
				</tr>
			</tbody>
		</table>
	</div>
...
let internamentoId = ref()


async function cadastrarInternamento() {
	const url = 'https://localhost:7203/cadastrar/internamento';

	if (dataInt.value == null || dataAlta.value == null || leito.value == null ||
		cid.value == null || carater_atendimento.value == null || motivo_Encerramento.value == null ||
		editar.value == null || internamentoId == null) {
...
async function editarInternamento() {
	const url = 'https://localhost:7203/atualizar/internamento/' + internamentoId.value;

	const dados = {
		internamentoId: Number(internamentoId.value),
		dataInt: String(dataInt.value),
		dataAlta: String(dataAlta.value),
		leito: String(leito.value),
		cid: String(cid.value),
...
function getInternanmentoId(data) {
	console.log(data.target.id)
	getInternanmento(data.target.id)
}
async function getInternanmento(id) {
	const req = await fetch('https://localhost:7203/buscar/internanmento/' + id)
	const res = await req.json()
	console.log(res)
	dataInt.value = res[0].dataInt
	dataAlta.value = res[0].dataAlta
	leito.value = res[0].leito
	cid.value = res[0].cid
	carater_atendimento.value = res[0].carater_Atendimento
	motivo_Encerramento.value = res[0].motivo_Encerramento
	internamentoId.value = Number(res[0].internamentoId)
	editar.value = true
}
